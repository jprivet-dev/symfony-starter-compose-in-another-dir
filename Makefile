...
.PHONY: aliases
aliases: ## Create aliases file (Generate and load aliases with: $ . scripts/aliases.sh)
	@printf "🚧 Create $(YELLOW)php$(RESET), $(YELLOW)composer$(RESET) & $(YELLOW)symfony$(RESET) aliases 🚧‍\n"
	@>$(ALIASES_SOURCE) # Clear all
	@echo '# File auto-generated by the aliases command in the Makefile ($(shell date +"%Y-%m-%d %H:%M:%S")).' >>$(ALIASES_SOURCE)
	@echo 'alias php="$(PHP)"' >>$(ALIASES_SOURCE)
	@echo 'alias composer="$(COMPOSER)"' >>$(ALIASES_SOURCE)
	@echo 'alias symfony="$(CONSOLE)"' >>$(ALIASES_SOURCE)
	@echo 'printf "🎉 All aliases loaded 🎉\\n"' >>$(ALIASES_SOURCE)
